...
const validationSchema = Yup.object().shape({
  ...
  navName: Yup.string()
    .required("Navigation Name is required")
    .matches(
      /^([A-Za-z0-9- ]){3,30}$/,
    ...
    if (!isURL(navLink, urlOptions)) {
      ...
      if (existingName?.length > 0) {
        ...
        if (navType == "Navigation Menu" && data?.length == 5) {
          ...
          openNotification(
            "danger",
            "Limit reached!! Maximum no of Navigation menu that can be created is 5"
          );
        } else if (navType == "Social Icon" && data?.length == 4) {
          setBtn(false);
          openNotification(
            "danger",
            "Limit reached!! Maximum no of Social icons that can be created is 4"
          );
        } else if (navType == "CTA Button" && data?.length == 1) {
          setBtn(false);
          openNotification(
            "danger",
            "Limit reached!! Maximum no of CTA Buttons that can be created is 1"
          );
        } else {
          const insertData = [
        ...
        {data?.map((nav) => (
          <Card bordered key={nav.id}>
            <Badge
              ...
              content={nav.type}
            />
            <h6 className="truncate mb-4 mt-5">{nav.name}</h6>
            <p className="truncate mb-4 font-semibold">
              {new Date(nav.created_at).toLocaleString("en-us", {
                month: "short",
                day: "numeric",
                year: "numeric",
              })}
