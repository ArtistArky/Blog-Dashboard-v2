...
import {
  ...
  setEmptyC,
  deleteCategoryData,
} from "store/userData/categorySlice";
...
const CategorySection = ({ data }) => {
  ...
  const deleteAction = async (id) => {
    ...
    const folderPath =
      "public/" + authID + "/" + id + "/featuredImg/" + "hd.jpeg";
    const folderPath2 =
      "public/" + authID + "/" + id + "/featuredImg/" + "sd.jpeg";

    await sbStorageDelete("category", [folderPath, folderPath2]).then(
      async ({ error, data }) => {
        if (error) throw openNotification("danger", error.message);
        if (data) {
          await sbUserDataDelete(
            "category",
            "id",
            id,
            "createdby",
            ...
            if (data) {
              ...
              const findIndex = editedCatdata.findIndex(
                (a) => a.id === data[0].id
              );
              ...
              catData.length === 0
                ? dispatch(setEmptyC())
                : dispatch(deleteCategoryData(data[0]));
              inPage = catData.length;
              fnPage = catData.length + maxCategory;
  ...
  const onCategoryDeleteConfirm = async () => {
    ...
    openNotification(
      "info",
      "Updating all the posts linked with the category...."
    );

    await sbUpdate(
      "posts",
      categoryId,
      { category: "255d4855-644e-43ab-829b-16adc417df97" },
      "category"
    ).then(({ error, data }) => {
      if (error) throw openNotification("danger", error.message);
      if (data) {
  ...
  const getFile = (files, form, field) => {
    ...
    openNotification(
      "info",
      "Featured Image conversion is under progress. Please wait before you submit the post"
    );
    const file = files[0];
    ...
    Compress(file, highresOptions)
      .then((compressedBlob) => {
        const convertedBlobFile = new File([compressedBlob], filehdName, {
          type: filetype,
        ...
        Compress(file, lowresOptions)
          .then((compressedBlob) => {
            const convertedBlobFile = new File([compressedBlob], filesdName, {
              type: filetype,
              lastModified: Date.now(),
            });
            ...
            openNotification(
              "success",
              "Featured Image conversion complete. You can now proceed to submit the post"
            );
    ...
    if (error) {
      ...
      await sbInsert("category", insertData).then(async (res) => {
        ...
        if (res.data) {
          ...
          openNotification(
            "success",
            "Uploading featured image. This may take a while. Please make sure you have stable internet connection"
          );
          ...
          for (var i = 0; i < images.length; i++) {
            ...
            await sbUpload("category", imagepath, images[i]).then(
              ({ error, publicURL }) => {
                if (error) {
                  setBtn(false);
                  openNotification("danger", error.message);
                ...
                if (publicURL) {
                  const mainurl =
                    publicURL.toString() + "?" + new Date().getTime();
          ...
          openNotification(
            "info",
            "Featured Image uploaded. Saving the details...."
          );
      ...
      <div className="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4 mt-4">
        ...
        {data?.map((category) => (
          <Card bordered key={category.id}>
            ...
            />
            <h6 className="truncate mb-4 mt-5">{category.name}</h6>
              ...
              <div className="flex">
                ...
                <Tooltip title="Edit">
                  <Button
                    ...
                    disabled={syncDisabled}
                    onClick={() => {
                      const { id, name, featured_imghd, featured_imgsd } =
                        category;
                      initFormState = {
                        catName: name,
        ...
        >
          <p>
            Are you sure you want to delete this category? All the posts under
            this category will not carry any category after delete. All this
