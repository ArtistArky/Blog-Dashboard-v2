...
import { HiOutlineMail, HiCheck, HiOutlinePlus } from "react-icons/hi";
...
const CreatePost = ({ data }) => {
  ...
  const [docsName, setdocsName] = useState(
    "Select your Google Docs from your Google Account"
  );
  ...
  const fetchCatList = async () => {
    ...
    await sbSelectDefault(
      ...
      if (data) {
        var catList = [
          { value: "255d4855-644e-43ab-829b-16adc417df97", label: "None" },
        ];
        data.map((item) => {
  ...
  const createPost = async (values) => {
    ...
    await checkDetails(postTitle, "posttitle", "posttitle", "posts").then(
      async (res) => {
        if (res.posttitle == postTitle) {
          setBtn(false);
          openNotification(
            "danger",
            "Title already exists. Choose a different title"
          );
        } else if (res.code && res.code == "PGRST116") {
          openNotification(
            "info",
            "Conversion of Google Docs file id to blog post is under process...."
          );
            ...
            .then(async (response) => {
              ...
              await sbInsert("posts", insertData).then(async (res) => {
                ...
                if (res.data) {
                  openNotification(
                    "info",
                    "Conversion of Google Docs file id to blog post completed. Uploading featured image. Please wait...."
                  );
                  ...
                  for (var i = 0; i < images.length; i++) {
                    ...
                    await sbUpload("posts", imagepath, images[i]).then(
                      ({ error, publicURL }) => {
                        if (error) {
                          setBtn(false);
                          openNotification("danger", error.message);
  ...
  return (
    <>
      <FormDesription title="Create New Post" desc="" />
      <div className="grid grid-flow-row-dense md:grid-cols-2 gap-4 mt-10 mb-10 cursor-pointer">
        ...
        <div
          ...
          className="col-span-2 mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-neutral-300 dark:border-neutral-700 border-dashed rounded-md"
        >
          <div className="space-y-1 text-center">
            ...
            />
            <p className="text-xs text-neutral-500">{docsName}</p>
          </div>
        </div>
      </div>
      {postmetaCon && (
