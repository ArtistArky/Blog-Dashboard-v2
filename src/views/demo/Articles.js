      ...
      if (data) {
        await sbUserDataDelete(
          ...
          if (data) {
            ...
            const findIndex = editedPostdata.findIndex(
              (a) => a.posttitle === data[0].posttitle
            );
            ...
            postData.length === 0
              ? dispatch(setEmpty())
              : dispatch(deletePostData(data[0]));
            dispatch(setEmptyC());
            inPage = postData.length;
            fnPage = postData.length + maxPosts;
  ...
  const editPost = async (values) => {
    ...
    if (images.length > 0) {
      for (var i = 0; i < images.length; i++) {
        const imagepath =
          "public/" + authID + "/" + postid + "/featuredImg/" + imagesName[i];
  ...
  return (
    <div className="mb-6">
      <div className="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4 mt-4">
        ...
        {data?.map((article) => (
          <Card className="w-50" bordered key={article.posttitle}>
            <Badge
              className="mr-4"
              content={article.category.name}
              innerClass="bg-blue-100 text-blue-700"
            />
            <h6 className="truncate mb-4 mt-5">{article.title}</h6>
        ...
        >
          <p>
            Are you sure you want to delete this post? Note:- This action cannot
            be undone.
          </p>
        </ConfirmDialog>
      </div>
